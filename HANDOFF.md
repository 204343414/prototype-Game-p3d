...
**交接保存状态**：本轮代码与文档仍在 OpenHands 工作区，未提交/推送；用户机器最新
运行代码在 `/tmp/prototype-map-preview.LRlPDWCg` 的隔离目录，主项目仍是 `8c3bacc`。
新聊天重新 clone 不会自动拿到这些未推送修改。先保存源码与交接再换会话；未经用户授权
不推送。不要把游戏派生 HTML、GLB、贴图、音频或 SSH 私钥混入提交。
