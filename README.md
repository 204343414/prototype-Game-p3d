...
- 实体货架可直接导出真正的二进制 FBX 7.4；
- FBX 包含网格、骨架、蒙皮、匹配的源动画及默认 `000_A_POSE_BIND`；
- 贴图以 Unity 稳定识别的同目录 PNG 交付，不依赖 Embedded Media；
- 实体可按当前类别/搜索结果批量导出，并指定安全的服务器输出路径；
- 已加载 Manhattan Cells 可下载 FBX 资源包或写入指定路径；
- `00audio.rcf` 全量 AudioFile/RADP 提取、WAV 写出和汇总工具已经公开；
- Alex 状态树模拟器、实体动画检查和地图工作台位于统一 viewer。

完整配置、目录布局、Unity 导入和已知限制见 **[`docs/fbx-export.md`](docs/fbx-export.md)**。
仓库不附带 Blender；FBX 转换需要本地 Blender 4.5 LTS，并通过
`PROTOTYPE_BLENDER` 指定路径。导出根目录通过 `PROTOTYPE_EXPORT_ROOT` 指定。

> **历史状态说明（2026-09-15 核对）**：Alex 全身蒙皮、骨架、贴图和
