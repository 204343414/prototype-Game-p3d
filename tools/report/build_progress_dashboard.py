...
<main class="main">
 <section class="page active" id="overview">
  <header class="top"><div><div class="eyebrow">Current verified foundation</div><h2>不是“导出了多少”，<br>而是“哪些已经可信”。</h2><p>这里是当前角色、动画和全地图考古的状态面板。城市仍处于静态几何/材质验证阶段；任何未验证的 UV、TRAN 或实例层都不会被伪装成完成。</p></div><div class="snapshot">SNAPSHOT<br><b id="snapshotTime">—</b><br><span id="snapshotSource">private local census</span></div></header>
  ...
  <div class="two-col">
   ...
   <article class="card"><h3>地图阶段门 <small>no false full-map claim</small></h3><p><b style="color:var(--cyan)">已确认：</b>260 个基础 Cell、149 个静态可渲染 Cell、111 个 placeholder，与用户提供的第三方 viewer 计数一致。</p><div class="callout"><b>下一个真实输出：</b>不是一次性倒出纽约，而是 2–4 个相邻 Cell 的带材质拼接预览。必须先通过接缝、UV、法线和 local-instance 隔离检查。</div></article>
  </div>
  <div class="two-col">
...
const milestones=[
 ...
 ['live','静态地图 vertex/material 映射','29 layouts；UV / normal 尚待渲染回归','active'],
