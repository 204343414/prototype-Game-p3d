...
.nav{{display:grid;gap:4px}} .nav button,.nav a{{border:0;background:transparent;text-align:left;padding:11px 12px;border-radius:7px;color:var(--muted);display:flex;align-items:center;gap:10px;text-decoration:none}}.nav button:hover,.nav a:hover{{background:#1a292d;color:var(--ink)}}.nav button.active,.nav a.active{{background:#203437;color:var(--acid);box-shadow:inset 2px 0 0 var(--acid)}}.nav .nav-num{{margin-left:auto;font:11px var(--mono);opacity:.7}}
...
.page{{display:block;animation:fade .22s ease;scroll-margin-top:18px}}@keyframes fade{{from{{opacity:.2;transform:translateY(5px)}}to{{opacity:1;transform:none}}}}
...
<aside class="sidebar">
 ...
 <nav class="nav" aria-label="主页导航">
  <a class="active" href="#overview">◈ 总览 <span class="nav-num">00</span></a>
  <a href="#map">▧ 曼哈顿 Cell <span class="nav-num">260</span></a>
  <a href="#shelf">▦ 带骨架物品栏 <span class="nav-num">491</span></a>
 </nav>
...
<main class="main">
 <section class="page active" id="overview">
  <header class="top"><div><div class="eyebrow">Archive home · verified boundaries</div><h2>先把已知成果，<br>做成能查、能选、能继续的主页。</h2><p>这里不是“完整地图已导出”的宣传页，而是当前真实证据的总入口：角色候选可筛选和勾选，曼哈顿可查看已扫描的世界 bounds，动画与格式工作按已验证、进行中、归档三种状态明确分开。无法稳定显示的 Cell WebGL 诊断不作为预览承诺。</p></div><div class="snapshot">SNAPSHOT<br><b id="snapshotTime">—</b><br><span id="snapshotSource">private local census</span></div></header>
...
const milestones=[
 ...
 ['warn','Cell 2 合并 core material diagnostic','结构拼装已完成；画面未稳定可见，归档而非预览承诺','archived'],
 ['warn','静态地图 normal / shader transform','等待已知可用的 renderer 线索或更强格式证据后再恢复','archived'],
...
// Navigation intentionally uses ordinary hash links, so all three archive sections
// remain reachable even if a future optional visualization widget fails.
</script></body></html>'''
