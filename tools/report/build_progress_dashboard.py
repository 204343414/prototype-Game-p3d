...
<main class="main">
 <section class="page active" id="overview">
  ...
  <div class="two-col">
   ...
   <article class="card"><h3>地图阶段门 <small>no false full-map claim</small></h3><p><b style="color:var(--cyan)">已确认：</b>260 个基础 Cell、149 个静态可渲染 Cell、111 个 placeholder，与用户提供的第三方 viewer 计数一致；Cell 2 / 3 的 92 个 core group、62,088 个三角形通过 POSITION / uint16 index 严格检查。</p><div class="callout"><b>下一个真实输出：</b>不是一次性倒出纽约，而是 2–4 个相邻 Cell 的带材质拼接预览。必须先通过接缝、UV、法线和 local-instance 隔离检查。</div></article>
  </div>
  <div class="two-col">
...
const milestones=[
 ...
 ['live','静态地图材质/UV 映射','本 Cell shader 定义已关联；UV / normal 待渲染回归','active'],
