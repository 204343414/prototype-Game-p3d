#!/usr/bin/env python3
"""
Alex Mercer 全身 (身体+头+外套+右臂+左臂, 多个 Composite_Drawable_2 部位组
合并) -> glTF (.glb) 一次性导出脚本。
...
用法:
    python3 export_alex_full.py --base-url https://xxxx.trycloudflare.com \
        --rcf-path "/mnt/hdd/新建文件夹/steamapps/common/Prototype/art.rcf" \
        --entry-name "\\art\\alex\\alex.p3d.rz" \
        --out alex_full.glb

设计取舍:
    - 默认合并导出 alex_reg_body (身体+头+外套, 3 Skin) + alex_reg_arms
      (右臂, 1 Skin) + alex_reg_left_arm (左臂, 1 Skin) 共 5 个 Skin,
      可用 --group-names 逗号分隔自定义要合并的部位组列表。
    - **关键简化 (已在 docs 里验证)**: alex_reg_body/alex_reg_arms/
      alex_reg_left_arm 三份骨架的 67 个关节名字、父子关系完全一致，仅
      Spine_1/Spine_2/Eye_R/Eye_L 4 个关节矩阵有 <=0.13 的微小浮点误差。
      因此全身导出只解析并使用 **第一个部位组** (通常是 alex_reg_body) 的
      骨架作为唯一/共享骨架，其余部位组的 Skin 直接把 mesh 挂在这份共享
      骨架上 (无需为每个部位组单独建一份 glTF skin/joint 节点树)。
      这个假设在导出时会做一次断言:每个部位组自己的 skeleton_name 对应的
      骨架 joint 数量必须等于共享骨架的 joint 数量 (67)，否则报错提醒
      需要单独处理 (说明未来遇到骨架结构差异更大的角色/部位时不能照搬)。
    - 骨骼绑定姿势 (bind pose) 直接取 Skeleton_Joint_2.rest_pose 级联算出的
      世界矩阵，不做任何姿势动画 —— 就是"T-pose/A-pose静止姿势"预览。
    - 贴图: 只导出 color(baseColor) 贴图，转 PNG 内嵌进 glb；normal/specular
      贴图的转码逻辑已写好但先保留在代码里、默认关闭
...
def main():
    ...
    ap.add_argument("--group-names", default="alex_reg_body,alex_reg_arms,alex_reg_left_arm",
                    help="逗号分隔的要合并导出的 Composite_Drawable_2 部位组名列表 "
                         "(默认身体+头+外套+右臂+左臂 = 全身)")
    ap.add_argument("--out", default="alex_full.glb")
    ap.add_argument("--with-normal-map", action="store_true")
    args = ap.parse_args()

    group_names = [g.strip() for g in args.group_names.split(",") if g.strip()]

    src = P3DSource(args.base_url, args.rcf_path, args.entry_name)
    print(f"[1/6] 定位 Composite_Drawable_2 组 {group_names} ...")
    ...
    for c in cd2_list:
        ...
        cd2_by_name[info["name"]] = (c, info)

    missing = [g for g in group_names if g not in cd2_by_name]
    if missing:
        raise SystemExit(f"没找到部位组 {missing}，可用: {sorted(cd2_by_name.keys())}")

    # 每个部位组各自拉取自己的 Skin (Composite_Drawable_Primitive) 名单，
    # 同时记录该组要求的骨架名 (用于后面做"共享骨架"一致性校验)。
    group_prim_names = {}   # group_name -> [prim_name, ...]
    group_skeleton_name = {}  # group_name -> skeleton_name
    for gname in group_names:
        cd2_chunk, cd2_info = cd2_by_name[gname]
        group_skeleton_name[gname] = cd2_info["skeleton_name"]
        ...
        group_prim_names[gname] = prim_names
        print(f"    组 '{gname}' (骨架={cd2_info['skeleton_name']}): "
              f"{len(prim_names)} 个 Skin: {prim_names}")

    # 全部 Skin 名字的合并列表 (按 group_names 顺序拼接)，用于后面统一取几何数据
    prim_names = [p for gname in group_names for p in group_prim_names[gname]]

    # ---- 共享骨架: 只用第一个部位组的骨架，参见文件头部"设计取舍"说明 ----
    shared_skeleton_name = group_skeleton_name[group_names[0]]
    print(f"[2/6] 定位共享骨架 '{shared_skeleton_name}' (取自第一个部位组 "
          f"'{group_names[0]}') 并解码骨骼层级 ...")
    ...
    for c in skel_list:
        ...
        if name == shared_skeleton_name:
            target_skel = c
            break
    if target_skel is None:
        raise SystemExit(f"没找到骨架 {shared_skeleton_name!r}")
    ...
    print(f"    解码出 {len(joints)} 根骨骼 (共享骨架)")

    # 校验: 其余部位组自己声明的骨架 joint 数量必须与共享骨架一致，否则说明
    # "三份骨架等价"这个假设对该角色/部位不成立，需要走单独骨架的方案。
    for gname in group_names[1:]:
        other_skel_name = group_skeleton_name[gname]
        if other_skel_name == shared_skeleton_name:
            continue
        other_skel_chunk = None
        for c in skel_list:
            b = bytes.fromhex(c["payload_hex_preview"])
            i = 0
            name, i = read_p3dstring(b, i)
            if name == other_skel_name:
                other_skel_chunk = c
                break
        assert other_skel_chunk is not None, f"没找到骨架 {other_skel_name!r}"
        other_header = parse_skeleton_2_header(
            bytes.fromhex(src.chunk_at(other_skel_chunk["global_index"], payload_preview=64)["payload_hex_preview"]))
        assert other_header["num_joints"] == skel_header["num_joints"], (
            f"部位组 '{gname}' 的骨架 '{other_skel_name}' 关节数 "
            f"({other_header['num_joints']}) 与共享骨架 '{shared_skeleton_name}' "
            f"({skel_header['num_joints']}) 不一致，不能直接共享蒙皮索引！"
            f"需要为该部位单独处理骨架。")
        print(f"    组 '{gname}' 骨架 '{other_skel_name}' joint 数量一致 "
              f"({other_header['num_joints']}) -> 可安全共享蒙皮索引")
