...
def export(data,out,name):
 ...
 # 共享包贴图与 camo/diffuseTexture 等通道别名现在由 entity_decoder 统一解析，
 # /api/entity_mesh 返回的 textures 已含 donor 条目（标记 donor:true），
 # material_texture_keys 也已填好 color —— 这里不再需要任何按 entity 的硬编码表。
