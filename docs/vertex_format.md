...
> **✅ UV朝向最终结论（曾经反复折腾过，此为定论，后续不要再改）**：
> 导出 glTF 时，UV 的 U/V 分量都应**原样直接使用**，不需要做任何翻转
> （既不用 `v = 1-v`，也不用交换U/V轴）。
>
> 踩坑史：第一版导出脚本（`export_alex_body.py`/`export_alex_full.py`）
> 曾加过一行 `uv[:, 1] = 1.0 - uv[:, 1]`，是凭"DirectX贴图坐标系→OpenGL/
> glTF坐标系习惯性翻转"的经验猜测加的，**没有真正验证过**，导致渲染出来
> 脸部/五官贴图位置错乱（眼睛发黑、嘴部比例畸变，像戴了张歪的面具）。
> 用户通过一个专门加到预览页里的"UV调试面板"（旋转90°/左右镜像/上下镜像
> 三个独立开关+实时状态显示）现场试出正确观感对应的参数组合，然后用
> 真实 three.js 里的 `texture.matrix`（而非手工心算）精确反推出这组参数
> 的等价采样公式，代入"导出代码已经翻转过一次v"的事实换算后，结论是
> "两次翻转刚好互相抵消" —— 即正确做法是**完全不翻转**，直接用游戏原始
> UV。已删除那行错误代码并重新导出验证：五官、白衬衫领口等细节均正确
> 无误。**教训：任何凭"行业惯例/经验猜测"加的坐标系转换代码，必须真正
> 拿实际渲染结果验证，不能想当然——尤其是U/V轴翻转、左右手坐标系这类
> "有50%概率蒙对"的地方，最省事可靠的验证方法就是像本次一样做一个可交互
> 调试面板让人眼判断，比反复纯代码走查更快更准。**
>
> 后续如果要导出其他角色/新的Skin（比如 Blackwatch/Hunter/E妈等），
> 理论上应遵循同样的"UV原样使用不翻转"规则，但**仍建议每个新角色第一次
> 导出后都用UV调试面板肉眼复核一次**，不要预设所有角色/所有shader都完全
> 一致（目前只在 Alex Mercer 身上验证过，样本量是1个角色5个Skin）。
...
- ~~`0x00010001` (Skin) 本体绑定关系~~ **已解码，见第8节**。
  ~~仍待确认：body/arms/left_arm 骨架是否等价~~ **已完整验证**：三份
  骨架（`alex_reg_body_skeleton`/`alex_reg_arms_skeleton`/
  `alex_reg_left_arm_skeleton`）均为67关节/13分区/4肢体，逐关节比对
  joint chunk的名字+父子关系+局部矩阵，三者几乎完全相同，仅
  `Spine_1`/`Spine_2`/`Eye_R`/`Eye_L` 4个关节矩阵有≤0.13的微小浮点
  误差，可视为同一骨架的等价副本。`tools/p3d_export/export_alex_full.py`
  已据此实现全身导出（统一用body骨架，arms/left_arm的mesh直接挂上去），
  并在运行时做了断言校验这个假设（换角色时如果关节数不一致会报错提醒，
  不会静默出错）。详见 `HANDOFF.md`"关键数据点"一节。
